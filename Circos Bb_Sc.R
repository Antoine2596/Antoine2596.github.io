...
colnames(Squalius) <- c("gene_name", "start","end","chr","align_length","eval","perc_id")
...
Barbus <- Barbus[,-c(5,6,7)]
Squalius <- Squalius[,-c(5,6,7)]



ensemble1 <- merge(Barbus, Squalius, by = "gene_name")
...
for (i in 1:50){
  ...
  Bbliens$chr.y[Bbliens$chr.y == i ] <- paste("Sc_", i, sep="")}




#Longueur chromosome Barbus barbus
library(dplyr)
Barbus_Longueur_Chromosome <- read.table("Fichier/Barbus_Longueurs_chromosome.txt")
colnames(Barbus_Longueur_Chromosome) <- c("ID","Length")

Barbus_Longueur_Chromosome <- Barbus_Longueur_Chromosome[grepl("^O", Barbus_Longueur_Chromosome$ID), ]

Barbus_Longueur_Chromosome <- Barbus_Longueur_Chromosome[-c(51),]
 
Barbus_Longueur_Chromosome <- Barbus_Longueur_Chromosome %>% 
  mutate(ID = paste0("Bb_", row_number()))


#Longueur chromosome Squalius cephalus
Squalius_Longueur_Chromosome <- read.table("Fichier/Squalius_Longueurs_chromosome.txt")
colnames(Squalius_Longueur_Chromosome) <- c("ID","Length")

Squalius_Longueur_Chromosome <- Squalius_Longueur_Chromosome[grepl("^C", Squalius_Longueur_Chromosome$ID), ]


Listes_chromosomes_Squalius <- Squalius_Longueur_Chromosome %>% 
  mutate(ID = paste0("Sc_", row_number()))


library(BioCircos) 
# On créer une liste contenant les noms et longueur des chromosomes avec le Chr 1 de Barbus barbus

Genome_Barbus_Squalius <- rbind(Barbus_Longueur_Chromosome,Listes_chromosomes_Squalius)




Genome_Barbus_Squalius <- setNames(as.double(Genome_Barbus_Squalius$Length), Genome_Barbus_Squalius$ID)

MyGenome <- as.list(Genome_Barbus_Squalius)
...
#Enregistrer le cirocs en .html
saveWidget(Circos, "Résultat/Circos_Bb_Sc.html", title = "Duplication_Bb") # "titre =..." permet d'avoir le titre dans le nom de la page web

#Convertir le .html en .pdf
chrome_print("Résultat/Circos_Bb_Sc.html", output = "Résultat/Circos_Bb_Sc.pdf", format="pdf",
             options=list(paperWidth=20, paperHeight=10, preferCSSPageSize=TRUE), extra_args = c("--window-size=2800,1200"))
